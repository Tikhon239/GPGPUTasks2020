...
int main()
{
    ...
    for (int platformIndex = 0; platformIndex < platformsCount; ++platformIndex) {
        ...
        //Код ошибки -30 соотвествует CL_INVALID_VALUE - if param_name is not one of the supported values or if size in bytes specified by param_value_size is less than size of return type and param_value is not a NULL value.
        // TODO 1.2
        ...
        for (int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex) {
            ...
            std::cout << "    Platform_device #" << (deviceIndex + 1) << "/" << devicesCount << std::endl;
            ...
            size_t deviceDriverNameSize = 0;
            OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DRIVER_VERSION, 0, nullptr, &deviceDriverNameSize));
            std::vector<char> deviceDriverName(deviceDriverNameSize, 0);
            OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DRIVER_VERSION, deviceDriverNameSize, deviceDriverName.data(), nullptr));
            std::cout << "        Device driver version: " << deviceDriverName.data() << std::endl;

            cl_ulong device_size_mem;
            OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_GLOBAL_MEM_SIZE, sizeof(cl_ulong), &device_size_mem, nullptr));
            std::cout << "        Size of global device memory in megabytes : " << device_size_mem/static_cast<cl_ulong>(1000000) << std::endl;

            cl_ulong device_size_cache_mem;
            OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_GLOBAL_MEM_CACHE_SIZE, sizeof(cl_ulong), &device_size_cache_mem, nullptr));
            std::cout << "        Size of global device memory cache in bytes : " << device_size_cache_mem << std::endl;
        }
    }
