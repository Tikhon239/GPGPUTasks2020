...
int main(int argc, char **argv)
{
    ...
    /*
    {
        ...
        ocl::Kernel tree_sum(radix_kernel, radix_kernel_length, "tree_sum");
        tree_sum.compile();
        ocl::Kernel prefix_sum_kernel(radix_kernel, radix_kernel_length, "prefix_sum");
        prefix_sum_kernel.compile();
        ocl::Kernel partial_sum_kernel(radix_kernel, radix_kernel_length, "partial_sum");
        partial_sum_kernel.compile();
        ocl::Kernel local_radix(radix_kernel, radix_kernel_length, "local_radix");
        local_radix.compile();

        unsigned int workGroupSize = 128;
        unsigned int number_of_groups = (n + workGroupSize - 1) / workGroupSize;
        unsigned int global_work_size =  number_of_groups * workGroupSize;

        gpu::gpu_mem_32u cur_as, next_as, initial_prefix_sum, prefix_sum, cur_partial_sum, next_partial_sum;
        cur_as.resizeN(n);
        next_as.resizeN(n);

        prefix_sum.resizeN(number_of_groups);
        cur_partial_sum.resizeN(number_of_groups);
        next_partial_sum.resizeN(number_of_groups);
        std::vector<unsigned int> zeros_array(number_of_groups, 0);
        initial_prefix_sum.resizeN(number_of_groups);

        timer t;
        for (int iter = 0; iter < benchmarkingIters; ++iter) {
            cur_as.writeN(as.data(), n);
            //можно ли как то покрасивее это делать?
            initial_prefix_sum.writeN(zeros_array.data(), number_of_groups);
            t.restart(); // Запускаем секундомер после прогрузки данных чтобы замерять время работы кернела, а не трансфер данных
            for(unsigned int bit = 0; bit <= std::log2(std::numeric_limits<int>::max())+1; ++bit) {
                tree_sum.exec(gpu::WorkSize(workGroupSize, global_work_size), cur_as, cur_partial_sum, n, bit);
                //хотим вычислить значение префиксов перед work group
                initial_prefix_sum.copyToN(prefix_sum, number_of_groups);
                unsigned int cur_size = number_of_groups;
                for (unsigned int pow = 0; pow <= std::log2(number_of_groups); ++pow) {
                    prefix_sum_kernel.exec(gpu::WorkSize(workGroupSize, std::max(number_of_groups, workGroupSize)), cur_partial_sum, prefix_sum, number_of_groups, pow);
                    cur_size /= 2;
                    partial_sum_kernel.exec(gpu::WorkSize(workGroupSize, std::max(cur_size, workGroupSize)), cur_partial_sum, next_partial_sum, cur_size);
                    next_partial_sum.copyToN(cur_partial_sum, cur_size);
                }

                local_radix.exec(gpu::WorkSize(workGroupSize, global_work_size), cur_as, next_as, prefix_sum, n, bit);
                t.nextLap();
                next_as.copyToN(cur_as, n);
            }
        }
